		...
		if imgui.tree_node_str_id(tostring(key) .. "T", display_name or EMV.ImguiTable.get_element_name(value, key) ) then
			...
			imgui.push_id(key .. "+")
				...
				if imgui.button("Add") and (not m_tbl[subtbl_key] or (m_tbl[subtbl_key].___new_value:sub(1,5) ~= "[New]")) then 
					...
					m_tbl[subtbl_key].___new_value = "[New]"
		...
		if m_subtbl.value ~= nil and m_subtbl.value ~= tostring(converted_value) then 
			imgui.push_id(key)
				if imgui.button("Set") then
					...
					if m_subtbl.value == "\"\";" and (not check_add_func or check_add_func(m_subtbl.value)) then
						owner_tbl[key] = "" --special cases to let you assign an empty string with ""; or '';
					elseif m_subtbl.value == "'';" and (not check_add_func or check_add_func(m_subtbl.value)) then
						owner_tbl[key] = ''
					else
						...
						elseif (m_subtbl.value=="") or (m_subtbl.value=="nil") or (m_subtbl.value=='') then --lets you delete any value with "" or "nil"
							if not owner_tbl.___is_array or not pcall(function() table.remove(owner_tbl, (key)) end) then
								owner_tbl[key] = nil
							end
							output = 1
						end
...
local function lua_get_system_array(sys_array, allow_empty, convert_to_table)
	...
	if system_array and ((system_array[1] and sdk.is_managed_object(system_array[1])) or (not system_array[1] and next(system_array) and sdk.is_managed_object(({next(system_array)})[2]))) then
		for key, element in pairs(system_array) do 
			if element then element:add_ref() end
		end
	end
	return system_array
end
...
deferred_call = function(managed_object, args, index, on_frame)
	...
	if not index and isArray(args) then
		...
		if managed_object and managed_object.get_type_definition then
			...
			if args.command then --commands as strings
...
local VarData = {
	...
	new = function(self, args, o)
		...
		if example ~= nil then --need one example before can start updating
			...
			o.is_vt = not o.is_lua_type and (not not (tostring(example):find("::ValueType")) or o.is_vt) or nil
			o.is_obj = (not o.is_lua_type and (not o.is_vt and sdk.is_managed_object(example))) or nil
			o.is_pos = o.is_lua_type == "vec" or nil
			o.is_rot = o.is_lua_type == "qua" or (o.is_lua_type == "vec" and (o.name:find("[Ee]uler") and 1)) or nil
					...
					if (old_value~=nil) and (old_value ~= out) and old_value._  then --and not self.array_count and is_obj_or_vt(out) then
						...
						if self.is_vt and not self.is_lua_type and old_obj and not self.mysize then 
							...
							for i, fdata in ipairs(new_tbl.field_data or {}) do 
								old_obj:set_field(fdata.field:get_name(), fdata.value)
							end
							for i, prop in ipairs(new_tbl.props or {}) do 
								if prop.set and not prop.mysize then
									prop.set:call(old_obj, prop.value)
								end
							end
							metadata[out] = nil --delete stupid new value
						elseif (old_value ~= nil) and (old_value ~= out) then]]
						metadata[old_value] = nil --clear old metadata for values being replaced
					--if old_value._ then
					--	old_value._.reset = out
					--	create_REMgdObj(out)
					end
				end
				if index then
...
add_to_REMgdObj = function(obj)
	...
	mt.__index = function(self, key)
		return ((_G[typename] and (key ~= "type" or typename ~= "ValueType"))  and _G[typename][key]) or REManagedObject[key] or (metadata[self] and metadata[self][key]) 
	end
...
if REMgdObj_objects then
	...
	mathex = (sdk.create_instance(tds.mathex:get_full_name(), true) or sdk.create_instance(tds.mathex:get_full_name(), true)):add_ref()
...
local function read_field(parent_managed_object, field, prop, name, return_type, key_name, element, element_idx)
	...
	if (value ~= nil) and not element_idx then 
		if imgui.button("+") then 
			var_metadata.show_var_data = not var_metadata.show_var_data or nil
		end
		imgui.same_line()
	end
		...
		if imgui.tree_node_str_id(key_name .. name, display_name) then 
			...
			for i=0, 3 do 
				...
				if i == 3 and new_value[i].w == 1 then 
					...
					--var_metadatal.is_pos = true is_pos, is_rot = name:find("Position"), name:find("Rotation")
				end
			end
			if was_changed then 
				value = new_value 
	...
	elseif (value and not (value.x or value[0])) and ((var_metadata.is_obj or var_metadata.is_vt) or (var_metadata.can_index and (value._ or tostring_value:find("::ValueType") or ({pcall(sdk.is_managed_object, value)})[2] == true))) then
		is_obj = true
		...
		if not count and not element_idx then
			...
			if return_type then
				--local return_type = (type(value.get_type_definition)=="function") and value:get_type_definition() or value.w and sdk.find_type_definition("via.vec4") or sdk.find_type_definition("via.vec3") --fukken hate these oddball lua types
		...
		if imgui.tree_node_str_id(key_name .. name, display_name .. (count and (" (" .. count .. ")") or "")) then
			...
			--[[if changed and var_metadata.is_vt then 
				changed = false
			end]]
			imgui.tree_pop()
	...
	if not is_obj and (((freeze ~= nil) and (field or prop.set) and (var_metadata.value_org ~= nil))) then --freezing / resetting values 
		...
		imgui.push_id(var_metadata.name or field:get_name() .. "F")
			...
			if freeze or (var_metadata.timer_start and (uptime - var_metadata.timer_start) > 0.1) then
				if (not var_metadata.display_name and not imgui.same_line() and imgui.button("X")) then
					if element_idx then 
	...
	if var_metadata.show_var_data then 
		...
		imgui.begin_rect()
			if do_offer_worldpos then 
				...
				if (world_tbl and world_tbl.active) or var_metadata.is_pos or var_metadata.is_rot then 
					imgui.same_line()
					local draw_changed
					...
					if var_metadata.draw_gizmo then
						...
						--mat[3] = value.to_vec4 and value:to_vec4() or o_tbl.pos or o_tbl.xform and o_tbl.xform:call("get_Position"):to_vec4() or value:to_vec3():to_vec4() or mat[3]
						if value.conjugate then 
							mat = value
						else
							if var_metadata.is_rot then
								mat = (value.to_mat and value:to_mat()) or value:to_mat4()
								--mat[3] = (o_tbl.pos and o_tbl.pos:to_vec4()) or Vector4f.new(0,0,0,1)
							end
							mat[3].w = 1.0
						end
						...
						--end
						--imgui.text(logv(mat))
						was_changed, mat = draw.gizmo(parent_managed_object:get_address() + (prop.index or 0), mat, (var_metadata.is_rot and imgui.ImGuizmoOperation.ROTATE) or (var_metadata.is_pos and imgui.ImGuizmoOperation.TRANSLATE) or nil, display_name:find("[Ll]ocal") and imgui.ImGuizmoMode.LOCAL or nil)--, imgui.ImGuizmoMode.WORLD)
						--imgui.text(logv(mat))
						changed = was_changed or changed
						if was_changed then
							value = (value.w and mat[3]) or mat[3]:to_vec3()
						end
					end
...
local function show_collection()
						...
						if changed then 
							...
							if cd.sel_obj and not cd.same_joints_constraint then --on selected:
								...
								if cd.gizmo_freeze_selected and obj.cog_joint then
									...
									deferred_calls[obj.cog_joint] = {func="set_LocalPosition", args=obj.cog_joint:call("get_BaseLocalPosition"), vardata=vd.obj._LocalPosition}
	...
	if not cd.g_menu_open then imgui.begin_rect() end
		if imgui.button((cd.g_menu_open and "-" or "+") .. " GameObject Spawner") then
					...
					imgui.begin_rect()
						cd.recurse = setup_comp_names and function(tbl)
							for k, v in pairs(tbl) do
								...
								if type(v)=="string" and (v:lower():find("error") or v:sub(1,4)=="res:") then 
									tbl[k] = nil
								end
							end
...
GameObject = {
	...
	new = function(self, args, o)
		...
		if not try or not o.gameobj or not is_valid_obj(o.xform) then
			if o.xform then 
				self.dead_addresses[o.xform] = true
			end
			return 
		end
		self.__index = self
	...
	imgui_xform = function(self)
		...
		if EMVSettings and not figure_mode and not cutscene_mode and self.layer and not self.forced_mode_center and not imgui.same_line() and imgui.button(forced_mode and "Add to Animation Viewer" or "Enable Animation Viewer") then
		...
		self.parents_list = self.parents_list or {names={}, names_to_xforms={}}
		...
		if not self.index_h_count or (self.index_h_count ~= htc) or (self.parent and (self.parent ~= self.parents_list.names_to_xforms[ self.parents_list.names[self.parent_index] ])) then 
			...
			imgui.text("Sorting")
			local uniques = {}
			sorted_held_transforms, self.parents_list = {}, {names={}, names_to_xforms={}}
		...
		imgui.same_line()
		imgui_changed, self.parent_index = imgui.combo("Set Parent ", self.parent_index, self.parents_list.names)
		...
		local parent_name = self.parents_list.names[self.parent_index]
	...
	update_components = function(self, basis_args)
		...
		local make_touched_gameobj = touched_gameobjects[self.xform]
		held_transforms[self.xform], touched_gameobjects[self.xform] = nil
		self = GameObject:new_AnimObject({xform=self.xform}, basis_args)
		if make_touched_gameobj then
			self = GameObject:new_GrabObject({xform=self.xform}, basis_args)
		end
		return self
	...
	update_all_joint_positions = function()
		for xform, obj in pairs(held_transforms) do 
			if obj.joints  then
				...
				local do_local = active_poser and obj.poser.is_local
...
GameObject.update_GameObject = GameObject.update
...
re.on_draw_ui(function()
	...
	if imgui.tree_node("EMV Engine Settings") then
		...
		changed, SettingsCache.cache_orderedPairs = imgui.checkbox("Cache Ordered Lists", SettingsCache.cache_orderedPairs); EMVSetting_was_changed = EMVSetting_was_changed or changed
