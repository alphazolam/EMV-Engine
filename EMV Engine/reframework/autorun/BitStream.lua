...
REResource = {
		...
		for i, fieldTbl in ipairs(struct or {}) do 
			...
			elseif methodName == "seek" then 
				if doInsert then
					...
					bs:seek(tableToWrite.startOf + keyOrOffset)
				end
			elseif methodName == "align" then
				if doInsert then
	...
	displayImgui = function(self)
		if self.filepath then
			if imgui.button("Save File") then
				self:save()
			end
			imgui.same_line()
			EMV.editable_table_field("filepath", self.filepath, self)
		...
		for i, structName in ipairs(self.structs.structOrder) do 
			...
			if pluralName and imgui.tree_node(pluralName) then
				...
				local function display_struct(s, struct, doExpand)
				...
				if pluralName==structName and structName ~= "objectTable" then
					...
					local thisInfos = self[pluralName]
					local isList
					--imgui.text(self.structs[structName][1][2])
					if thisInfos and not thisInfos[ self.structs[structName][1][2] ] then
					--	isList = true
					end
										...
										if imgui.tree_node("List (" .. field.fieldTypeName .. ") " .. field.name) then
											...
											for e, element in ipairs(field.value) do
												...
												if type(element)=="table" and field.objectIndex then
													displayInstance(element, e .. ". " .. field.fieldTypeName .. " " .. field.name, field, e)
					...
					if imgui.tree_node(displayName) then 
						...
						if not gameObject.parents_list or not gameObject.imguiParentIdx or self.gameobjTableResetAction then 
							...
							for i, gInfo in ipairs(self.gameObjectInfos) do 
								...
								if gInfo.objectId == gameObject.gInfo.parentId or (gInfo==gameObject.gInfo and not gameObject.parent) then 
...
RSZFile = {
	...
	sortRSZField = function(self, field, instance)
		...
		sortedField.objectIndex = (field.isNative and (field.LuaTypeName=="Int" and type(fieldValue)=="number") and (fieldValue < instance.index-1) and (fieldValue > instance.index - 101)) or nil
...
SCNFile = {
	...
	save = function(self, filepath, onlyUpdateBuffer)
		...
		for i, folderInfo in ipairs(self.folderInfos) do
		...
		for i, prefabInfo in ipairs(self.prefabInfos) do
...
PFBFile = {
	...
	read = function(self, start)
		...
		if self.RSZ.objectTable then
			...
			local gameObjParentMap = {}
			for i, info in ipairs(self.gameObjectInfos) do 
				local gameObject = { gameobj = self.RSZ.rawData[ self.RSZ.objectTable[info.objectId+1].objectId ].sortedTbl, components={}, children={}, idx=i }
				for j=info.objectId + 2, (info.objectId + info.componentCount + 1) do
					table.insert(gameObject.components, self.RSZ.rawData[ self.RSZ.objectTable[j].objectId ].sortedTbl)
				end
				gameObjParentMap[info.objectId] = gameObject
				...
				info.name = gameObject.gameobj[1].value
